...
designs.each do |design|
  ...
  raise "no promoter found with: #{design.fpu_biofab_id}" if !part
  design.fpu_part_id = (part) ? part.id : nil

  part = Part.find_by_biofab_id(design.cds_biofab_id)
  raise "no 5' UTR found with: #{design.cds_biofab_id}" if !part
...
puts
puts "== Mean-centering fc-data by per-GOI means and log2'ing =="
