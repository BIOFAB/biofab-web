...
#      puts " "
  ...
  def get_characterization_xls(out_path=nil)
    ...
    error_sheet = xls_add_plate_sheet(workbook, 'Errors')
    ...
    wells.each do |well|
      ...
      error_txt = well.replicate.characterization_description('error')
      if !error_txt.blank?
        error_sheet[well.row.to_i, well.column.to_i] = error_txt
      end
